...
//Facebook Payload Objects - Examples
const FP_QuickReplie = [
  {
      title: "Si",
...
async function saveUserData(facebookId) {
  ...
  fetch(url, params).then(response => {
    ...
    console.log("RESPUESTA DE FETCH EN SAVE USER DATA");
    console.log(json);
  });
}
...
async function handleDialogFlowAction(
  ...
  console.log("-------ENTRO AL MENEJADOR DE ACCIONES-------");
  switch (action) {
    case "catalogo.action":
      let tipoFunko = parameters.fields.tipoFunko.stringValue;
      let FunkoNombre = parameters.fields.FunkoNombre.stringValue;
      console.log(parameters);
      if (tipoFunko != null && tipoFunko != "")
      {
        let url = bdApi + '/products?productType=' + tipoFunko + '&type=1';
        let params = {
          method: "GET",
        ...
        fetch(url, params).then(response => {
          ...
          let targetasMenu = [];
          let funkosBD = json.products;
          funkosBD.forEach(funko => {
            targetasMenu.push({
              title: funko.name + ' $' + funko.price,
              image_url: funko.img,
              subtitle: funko.description,
              buttons:[
                {
                  type: "postback",
                  title: "Comprar",
          ...
          sendTextMessage(sender, "Claro!  📝  A continuación te muestro los Funko Pop de " + tipoFunko);
          handleMessages(messages, sender);
          sendGenericMessage(sender, targetasMenu);
        });
      }
      else if (FunkoNombre != null && FunkoNombre != "")
      {
        let url = bdApi + '/products?productVar=' + FunkoNombre + '&type=2';
        let params = {
          method: "GET",
          headers: { "Content-Type": "application/json" }
        };
        ...
        fetch(url, params).then(response => {
          ...
          sendTextMessage(sender, "Claro!  📝  A continuación te muestro los Funko Pop de " + FunkoNombre);
          handleMessages(messages, sender);
          sendGenericMessage(sender, targetasMenu);
        });
      }
      else if ((FunkoNombre == null && tipoFunko == null) || (FunkoNombre == "" && tipoFunko == ""))
      {
        sendTextMessage(sender, "Claro! ¿Que Funko Pop deseas? Actualmente manejamos Pops de Anime,  Disney, Marvel, DC, Start Wars, Cartoons, Video Juegos, Musica, Peliculas y de Deportistas");
        handleMessages(messages, sender);
      }
      break;

    case "FinalizarVenta.action":
      //Agregar función para guardar numero telefonico y dirección
      let location = parameters.fields.location.stringValue;
      let phone = parameters.fields.phoneNumber.stringValue;
      let userData = await getUserData(sender);
      sendTextMessage(sender, "Te estaremos contactando por telefono para confirmar tu pedido");
      sendTextMessage(sender, "Gracias " + userData.first_name + "! por tu compra 😁");
      sendTextMessage(sender, "Ayúdame a contestar una brebe encuesta. Te prometo que no tardarás: <link>");
      handleMessages(messages, sender);
      break;

    /*
    case "quickReplie.action":
      sendQuickReply(sender, "Ejemplo de Quick Reply ¿Estas de acuerdo?", FP_QuickReplie);
      break;
    */
    case "Ubicacion.action":
      sendTextMessage(sender, "Recuerda que hacemos envíos a toda Guatemala! 📦");
      handleMessages(messages, sender);
      break;
